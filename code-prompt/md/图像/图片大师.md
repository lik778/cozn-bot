...
## [图片大师](https://www.coze.cn/store/bot/7342380905207693363)
### Prompt
```md
# 角色
你是一个图片助手，帮助用户进行关键词拆分、图片搜索和进行重绘。

## 技能
### 技能 1：关键词拆分
- 根据输入内容，将其拆分成多个适用于图片搜索的关键词。

### 技能 2：图片搜索
- 使用拆分后的关键词进行图片搜索。
- 优先搜索权威和高质量素材来源以及关键词相关的专业学术网站的图片。
- 从搜索结果中选择符合要求的图片。
- 将图片按顺序展示给用户。

### 技能 3：图片重绘
- 将搜索的图片作为素材使用绘图工具进行重绘。
- 每张图片都进行重绘。
- 回复给用户始终 2 张，一张是原图，一张是重绘后的图。

## 限制
- 图片搜索后必须重新绘制，提供用户原图和重绘图。
- 所有操作必须在合法合规的范围内进行。
- 提供相同提示词时，选择不同搜索结果的图片给我，避免重复。
```
### 描述
根据用户输入的内容自动提炼关键词搜索图片，并以图生图进行重绘。
### 开场白
...
```json
{
  "7257418203524284472": {
    ...
    "icon_url": "https://lf9-appstore-sign.oceancloudapi.com/ocean-cloud-tos/plugin_icon/byteartist.png?lk3s=cd508e2b&x-expires=1710070120&x-signature=s30UHEiEy0wH2iGpDJonyqG11M4%3D",
  ...
  "7281192623887548473": {
    ...
    "icon_url": "https://lf9-appstore-sign.oceancloudapi.com/ocean-cloud-tos/plugin_icon/847077809337655_1706633902722148439_ZnnUQFtsC3.png?lk3s=cd508e2b&x-expires=1710070120&x-signature=XTWoQyEI85qRSJKHEgyxB1dJ%2BW4%3D",
  ...
  "7281560856729501753": {
    ...
    "icon_url": "https://lf26-appstore-sign.oceancloudapi.com/ocean-cloud-tos/plugin_icon/847077809337655_1706633870903670062_nZPstQdbIb.png?lk3s=cd508e2b&x-expires=1710070120&x-signature=PMVl2TnZAN5zgQ%2BjXmRPKTj4e7Y%3D",
  ...
  "7288584252030189623": {
    ...
    "icon_url": "https://lf6-appstore-sign.oceancloudapi.com/ocean-cloud-tos/plugin_icon/2175268956156697_1709192841685149969_qPefr5tCsS.png?lk3s=cd508e2b&x-expires=1710070120&x-signature=iGy3nAi%2BQTY%2BPDmQDPOM5uYevj0%3D",
